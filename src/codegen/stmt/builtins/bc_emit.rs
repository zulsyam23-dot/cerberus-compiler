...
pub(super) fn emit_bc_emit(
    ...
    if name == "bc_emit_print_str" {
        if !args.is_empty() && args.len() != 1 {
            return Err(CompileError::new_simple(
                "bc_emit_print_str expects 0 or 1 argument",
            ));
        }
        if !args.is_empty() {
            emit_expr(cg, &args[0])?;
    ...
    if name == "bc_emit_const_int" {
        if !args.is_empty() && args.len() != 1 {
            return Err(CompileError::new_simple(
                "bc_emit_const_int expects 0 or 1 argument",
            ));
        }
        if !args.is_empty() {
            emit_expr(cg, &args[0])?;
    ...
    if name == "bc_emit_store0" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_store0 expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_load0" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_load0 expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_println" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_println expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_const_bool" {
        if !args.is_empty() && args.len() != 1 {
            return Err(CompileError::new_simple(
                "bc_emit_const_bool expects 0 or 1 argument",
            ));
        }
        if !args.is_empty() {
            emit_expr(cg, &args[0])?;
    ...
    if name == "bc_emit_const_str" {
        if !args.is_empty() && args.len() != 1 {
            return Err(CompileError::new_simple(
                "bc_emit_const_str expects 0 or 1 argument",
            ));
        }
        if !args.is_empty() {
            emit_expr(cg, &args[0])?;
    ...
    if name == "bc_emit_load" {
        if !args.is_empty() && args.len() != 1 {
            return Err(CompileError::new_simple(
                "bc_emit_load expects 0 or 1 argument",
            ));
        }
        if !args.is_empty() {
            emit_expr(cg, &args[0])?;
    ...
    if name == "bc_emit_store" {
        if !args.is_empty() && args.len() != 1 {
            return Err(CompileError::new_simple(
                "bc_emit_store expects 0 or 1 argument",
            ));
        }
        if !args.is_empty() {
            emit_expr(cg, &args[0])?;
    ...
    if name == "bc_emit_strlen" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_strlen expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_concat" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_concat expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_substr" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_substr expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_replace" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_replace expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_vec_new" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_vec_new expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_vec_len" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_vec_len expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_vec_get" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_vec_get expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_vec_set" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_vec_set expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_vec_push" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_vec_push expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_vec_remove" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_vec_remove expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_vec_last" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_vec_last expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_vec_pop" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_vec_pop expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_map_new" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_map_new expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_map_len" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_map_len expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_map_set" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_map_set expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_map_get" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_map_get expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_map_has" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_map_has expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_map_remove" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_map_remove expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_readfile" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_readfile expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_writefile" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_writefile expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_arg_count" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_arg_count expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_str_clear" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_str_clear expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_vec_clear" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_vec_clear expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_map_clear" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_map_clear expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_env_get" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_env_get expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_env_has" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_env_has expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_path_join" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_path_join expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_fs_exists" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_fs_exists expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_fs_listdir" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_fs_listdir expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_now_timestamp" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_now_timestamp expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_load_op" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_load_op expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_store_op" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_store_op expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_println_op" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_println_op expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_writefile_op" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_writefile_op expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_add_op" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_add_op expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_sub_op" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_sub_op expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_mul_op" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_mul_op expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_div_op" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_div_op expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_eq_op" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_eq_op expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_ne_op" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_ne_op expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_lt_op" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_lt_op expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_le_op" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_le_op expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_gt_op" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_gt_op expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_ge_op" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_ge_op expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_and_op" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_and_op expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_or_op" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_or_op expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_not_op" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_not_op expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_neg_op" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_neg_op expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_strlen_op" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_strlen_op expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_concat_op" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_concat_op expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_substr_op" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_substr_op expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_replace_op" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_replace_op expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_vec_new_op" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_vec_new_op expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_vec_len_op" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_vec_len_op expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_vec_get_op" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_vec_get_op expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_vec_set_op" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_vec_set_op expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_vec_push_op" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_vec_push_op expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_vec_remove_op" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_vec_remove_op expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_vec_last_op" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_vec_last_op expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_vec_pop_op" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_vec_pop_op expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_vec_clear_op" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_vec_clear_op expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_map_new_op" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_map_new_op expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_map_len_op" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_map_len_op expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_map_set_op" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_map_set_op expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_map_get_op" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_map_get_op expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_map_has_op" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_map_has_op expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_map_remove_op" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_map_remove_op expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_map_clear_op" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_map_clear_op expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_readfile_op" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_readfile_op expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_arg_count_op" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_arg_count_op expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_arg_op" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_arg_op expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_env_get_op" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_env_get_op expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_env_has_op" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_env_has_op expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_cwd_op" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_cwd_op expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_path_join_op" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_path_join_op expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_fs_exists_op" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_fs_exists_op expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_fs_listdir_op" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_fs_listdir_op expects 0 arguments",
            ));
        }
    ...
    if name == "bc_emit_now_timestamp_op" {
        if !args.is_empty() {
            return Err(CompileError::new_simple(
                "bc_emit_now_timestamp_op expects 0 arguments",
            ));
        }
